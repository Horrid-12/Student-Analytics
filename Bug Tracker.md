...
| P3 | ✅ BUG-098 | Fixed | UI | Faculty Workspace text is hardcoded in sidebar brand | Sidebar/account identity is now context-driven (`_base_context` supplies `brand_edition`, `auth_role`, `auth_user`, `auth_status`, `auth_footer`); base.html + settings.html read them, so Phase 4.7 auth only flips context values. Commit 610de03; covered by `TestSidebarIdentityContext`. |
...
| ✅ BUG-095 | Fixed  | API      | HTTP 403 and 429 rate-limit responses were cached in `github_client.py`, causing rate limits to persist even after GitHub's limit reset. Fix: `github_client.py` only caches successful (200) and user-not-found (404) responses. Rate-limit and error responses are never cached. Before: rate limit lock-out persisted across resets. After: fresh requests succeed as soon as GitHub limits reset.                                                                                                                                                                                                                                                  |
| ✅ BUG-096 | Fixed  | API      | `check_rate_limit_parts` only checked `status_code == 403`, so HTTP 429 responses (GitHub secondary rate limit after retries exhausted) silently degraded the analysis — repos marked "unavailable", contributions lost — instead of surfacing `RateLimitError` to the user. Fix: added `if status_code == 429: raise RateLimitError(...)`. Before: 429 caused silent data loss. After: proper rate-limit error shown to user.                                                                                                                                                                                                                         |
| ✅ BUG-097 | Fixed  | API      | Search API rate limit (30 req/min authenticated, 10/min unauthenticated) caused batches to freeze and crash when fetching PR/issue counts for rosters. Fix: Search API rate limits are caught as per-user `unavailable_users` instead of aborting the batch. Removed blocking 60s sleep; reduced BATCH_SIZE to 15 with starting/completion logging in overview.html. Before: batch hung at 0/735 for minutes then crashed. After: batches process in seconds, updating progress smoothly.                                                                                                                                                              |
