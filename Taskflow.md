...
- [ ] 3.8 Deploy to free host (Vercel Python functions if FastAPI confirmed; Railway/Render otherwise) — **in progress 2026-08-29:** pushed Phase 3 port to `main` (`8f3b6b0`, 7,670 insertions) and imported into Vercel; first deploy crashed `FUNCTION_INVOCATION_FAILED` (expected — Vercel installs only `requirements.txt`, which was still the frozen Streamlit set, so `fastapi`/`mangum`/`plotly` were missing). Fix shipped: `requirements.txt` rewritten with the NEW STACK pins (original Streamlit set preserved exactly as `requirements.txt.dead` for restore — this pulls the 3.10 deps flip in early), and `vercel.json` added (`functions.api/index.py` 1 GB / `maxDuration` 60 to give batched analysis headroom + `rewrites` `/(.*)` → `/api/index` for all-route serving). **Consequence flagged:** the live Streamlit Community Cloud app rebuilds from this repo on push — once the new `requirements.txt` is on `main` its next rebuild has no `streamlit`, so the legacy deploy breaks the moment Streamlit Cloud notices. Recovery = restore `requirements.txt.dead`. Local venv already has all new deps (both suites still pass); `pip install -r requirements.txt` boot-smoke = 200. **Second deploy debug 2026-08-29:** wrote a Mangled ASGI wrapper in `api/index.py` — a plain `_StripVercelPrefix` middleware restores the original path after Vercel's `/(.*)` → `/api/index` rewrite (which otherwise delivers `path=/api/index` to the function, so FastAPI 404'd every real route). Covered by 4 new entrypoint tests (root, pages, unprefixed, static). **Third deploy debug 2026-08-29:** deployed `api/index.py` round (prefix expansion to `/api/index.py` + `_not_found` request-path echo, `77d25bd`) and Vercel's runtime logs exposed the REAL root cause — the Python runtime auto-detected the repo as a **Streamlit project** (root `app.py` present) and installed its own curated "runtime dependencies" (`streamlit==1.62.0`, `pydeck`, `pyarrow==24.0.0`, `pillow`, `numpy` … — see "Installed 40 packages" in the build log), WITHOUT fastapi/plotly/mangum/httpx, so every function crashed `ModuleNotFoundError: fastapi` (`Using cached runtime dependencies` = that stale layer reused across builds). The earlier hand-written `requirements.txt` swap was simply never what Vercel actually installed. **Fix:** executed the 3.10 cutover deletions NOW — `git rm app.py .streamlit/config.toml runtime.txt` (kills Streamlit framework detection; legacy test fixtures/components stay), and `vercel.json` now sets `"framework": null` (disable auto-detection) + `"installCommand": "pip install -r requirements.txt"` (explicit dep source). Legacy `services.py`/`storage.py`/`ui_helpers.py`/`style.css` stay in-repo as frozen parity references for the two-mode test suite.
- [ ] 3.9 Parity pass: real roster, click through every page/chart/export side-by-side against legacy
- [ ] 3.10 Cutover: delete `app.py`, `.streamlit/`, `runtime.txt`, Streamlit deps; pin the new `requirements.txt` — **deps flip executed early 2026-08-29** (see 3.8 note): `requirements.txt` now = new stack, original pinned as `requirements.txt.dead`. Remaining here: `app.py`, `.streamlit/config.toml`, `runtime.txt` **deleted 2026-08-29** (pulled forward as the Vercel Streamlit-misdetection fix — see 3.8); remaining: the actual cutover switch of which host is `main`-deployed (delete the legacy Streamlit Community Cloud project + confirm Vercel is canonical).
