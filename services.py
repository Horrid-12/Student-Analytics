...
class AnalysisResult:
    ...
    status: str
    # Team activity (group-project fix): external-repo pushes derived from the
    # public events API so members contributing to a leader-owned repo get
    # credit. Defaults keep old pickle/call sites working.
    team_summary_df: pd.DataFrame | None = None
    team_repos_df: pd.DataFrame | None = None
    team_unavailable_users: list[str] | None = None
...
def extract_username(text):
    ...
    if "/" in text or ("." in text and " " not in text):
        ...
        # Isolate the first URL token before parsing so
        # "https://github.com/Ritquehttps://..." still yields "Ritque".
        try:
...
def _first_url_token(text: str) -> str:
    ...
    # Fix doubled scheme typos seen in the Sep-2026 dump.
...
def extract_linkedin_username(text):
    ...
    if not match:
        # Bare slug (no URL)? Accept only clean single tokens.
...
def _reconcile_team_commits(
    ...
    count and the 30/90-day splits; one ``GET /repos/{full}`` fixes the
    Unknown / 0-stars display and Top Languages. API failures propagate
    RateLimitError but otherwise fall back to event values (caller swallows
    them).
    """
    if not rows:
        return summary, rows
...
def build_dashboard_df(
    ...
    # Prefer the cleaned form URL when the API profile URL is missing.
...
def run_analysis(
    ...
    def team_progress(index: int, total: int, username: str) -> None:
        if progress_callback:
            progress_callback("team_activity", index, total, username)

    team_summary_df, team_repos_df, team_unavailable_users = fetch_team_activity(
        github_stats["GitHub_Username"] if not github_stats.empty else [],
        token,
        team_progress,
    )
    ...
    def commit_progress(index: int, total: int, username: str) -> None:
        if progress_callback:
            progress_callback("commits", index, total, username)

    commit_summary_df, repo_df, commit_unavailable_users = fetch_owned_commit_data(
        repo_df,
        token,
        commit_progress,
    )
    ...
    return AnalysisResult(
        ...
        team_unavailable_users=team_unavailable_users,
    )
